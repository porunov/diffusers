...
def save_model_card(
    ...
    model_description = f"""
# Ideogram4 DreamBooth LoRA - {repo_id}
...
pipeline = Ideogram4Pipeline.from_pretrained("{base_model}", torch_dtype=torch.bfloat16)
pipeline.to('cuda')
...
Please adhere to the licensing terms of the base model [{base_model}](https://huggingface.co/{base_model}).
"""
...
def log_validation(
    ...
    # The base may be quantized (nf4): casting a quantized pipeline to a dtype is unsupported, and
    # from_pretrained already loads the components in the right dtype, so we skip the dtype cast here.
    # We move the whole pipeline to device rather than enable_model_cpu_offload(): the pipeline's
    # encode_prompt reaches into the text-encoder submodules directly, so the offload hooks never fire
    # and leave the encoder on CPU (device-mismatch on token embedding).
...
def module_filter_fn(mod: torch.nn.Module, fqn: str):
    # Keep precision-sensitive modules in higher precision: the final output projection
    # (final_layer.linear) and the modules Ideogram4Transformer2DModel flags in
    # `_skip_layerwise_casting_patterns` (timestep embedding, AdaLN projection, image-indicator embed).
    skip_patterns = ("final_layer.linear", "t_embedding", "adaln_proj", "embed_image_indicator")
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Rewrite each caption into Ideogram4's structured JSON caption (via the pipeline's prompt enhancer) "
            "before encoding/caching. Loads the prompt-enhancer LM head (see --prompt_enhancer_head_id)."
        ),
    )
    parser.add_argument(
        "--prompt_enhancer_head_id",
        type=str,
        default="diffusers/qwen3-vl-8b-instruct-lm-head",
        help=(
            "Repo id of the Ideogram4 prompt-enhancer LM head, loaded as the pipeline's `prompt_enhancer_head` "
            "component when --upsample_prompt is set."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=2048,
        help=(
            "Maximum text token length for prompt encoding (Ideogram 4 uses a Qwen3-VL text encoder; the pipeline "
            "default is 2048). Upsampled JSON captions can be long, so keep this large when using --upsample_prompt."
        ),
    )
    # parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            'Defaults to "logit_normal" to match the Ideogram4 schedule (resolution-aware mean + `--logit_std`). '
            "The sampled density is used directly as the flow-matching sigma."
        ),
    )
    parser.add_argument(
        "--logit_mean",
        type=float,
        default=0.0,
        help="Base mean of the logit-normal sigma schedule, before the resolution-aware shift "
        "(0.5*log(pixels/512**2)). Matches the pipeline's `mu`.",
    )
    parser.add_argument(
        "--logit_std",
        type=float,
...
def main(args):
    ...
    # Prompt upsampling needs the optional LM head grafted onto the (shared) text encoder; load it up front so the
    # text-encoding pipeline can rewrite captions into Ideogram4's structured JSON schema during caching.
    prompt_enhancer_head = None
    if args.upsample_prompt:
        try:
            from diffusers import Ideogram4PromptEnhancerHead
        except ImportError:
            raise ValueError(
                "`--upsample_prompt` requires a diffusers with Ideogram4 prompt upsampling "
                "(`Ideogram4PromptEnhancerHead`); please upgrade diffusers."
            )
        prompt_enhancer_head = Ideogram4PromptEnhancerHead.from_pretrained(args.prompt_enhancer_head_id)

    # Initialize a text encoding pipeline and keep it to CPU for now.
    text_encoding_pipeline = Ideogram4Pipeline.from_pretrained(
        args.pretrained_model_name_or_path,
        vae=None,
        transformer=None,
    ...
    if args.upsample_prompt and getattr(text_encoding_pipeline, "prompt_enhancer_head", None) is None:
        raise ValueError(
            "`--upsample_prompt` was set but the prompt-enhancer head failed to load; check "
            "`--prompt_enhancer_head_id` and your diffusers version."
        )
    ...
    if prompt_enhancer_head is not None:
        # The head is grafted onto the text-encoder body and consumes its hidden states, so it must share the body's
        # (floating) dtype — e.g. the bf16 a bnb-quantized text encoder outputs, regardless of mixed precision.
        prompt_enhancer_head.to(dtype=text_encoder.dtype)
    ...
    # Pixel size of one packed image token (vae_scale_factor * patch_size).
    patch_pixels = vae_scale_factor * patch_size
    latents_bn_mean = vae.bn.running_mean.view(1, 1, -1).to(accelerator.device)
    latents_bn_std = torch.sqrt(vae.bn.running_var + vae.config.batch_norm_eps).view(1, 1, -1).to(accelerator.device)
    ...
    # A pre-quantized checkpoint (e.g. nf4) is already quantized even without
    # --bnb_quantization_config_path; casting a quantized model to a dtype is unsupported, so only
    # set dtype for an unquantized transformer.
    transformer_is_quantized = args.bnb_quantization_config_path is not None or getattr(
    ...
    # The nf4 checkpoint ships a pre-quantized text encoder too; casting a bitsandbytes model to a
    # dtype is unsupported, so only move it to device (skip the dtype) when it is quantized.
    text_encoder_is_quantized = (
        getattr(text_encoder, "is_quantized", False)
        or getattr(text_encoder, "is_loaded_in_4bit", False)
        or getattr(text_encoder, "is_loaded_in_8bit", False)
    )
    if text_encoder_is_quantized:
        if not args.offload:
    ...
    if args.lora_layers is not None:
        ...
        # Default: LoRA on each Ideogram4 block's attention projections (to_q/to_k/to_v/to_out.0) and
    ...
    def load_model_hook(models, input_dir):
        ...
        if not is_fsdp:
            ...
            transformer_ = Ideogram4Transformer2DModel.from_pretrained(
                args.pretrained_model_name_or_path,
                subfolder="transformer",
            )
            transformer_.add_adapter(transformer_lora_config)

        lora_state_dict = Ideogram4Pipeline.lora_state_dict(input_dir)
    ...
    def compute_text_embeddings(prompt, grid_h, grid_w, text_encoding_pipeline):
        # encode_prompt builds the full packed conditioning for the given image grid and returns
        # (prompt_embeds, position_ids, segment_ids, indicator) ready to feed the transformer.
        with torch.no_grad():
            if args.upsample_prompt:
                # Ideogram4 is conditioned on a structured JSON caption; rewrite the caption(s) into that schema
                # (matching the image aspect ratio) before encoding. Done once here, then cached.
                # The enhancer grafts the LM head onto the (shared) text-encoder body, so the head must sit on the
                # same device as that body for the call (the body may already be on GPU — e.g. a bnb-quantized text
                # encoder — even when offloading is off).
                with offload_models(
                    text_encoding_pipeline.prompt_enhancer_head,
                    device=text_encoding_pipeline.text_encoder.device,
                    offload=True,
                ):
                    prompt = text_encoding_pipeline.upsample_prompt(
                        prompt, height=grid_h * patch_pixels, width=grid_w * patch_pixels
                    )
            prompt_embeds, position_ids, segment_ids, indicator = text_encoding_pipeline.encode_prompt(
                prompt=prompt,
                grid_h=grid_h,
                grid_w=grid_w,
    ...
    def patchify_latents(z, patch_size):
        """Inverse of Ideogram4Pipeline._decode (w/o the bn de-normalization): pack a VAE latent
        (B, ae_channels, H, W) into the transformer's token layout (B, grid_h*grid_w, ae*patch**2)."""
        batch_size = z.shape[0]
        patch = patch_size
        # z shape: (batch_size, ae_channels, H, W)
    ...
    # The packed conditioning depends on the image grid, so for the single instance prompt we use the
    # fixed training-resolution grid (single-bucket training).
    static_grid_h = static_grid_w = args.resolution // patch_pixels
    if not train_dataset.custom_instance_prompts:
        with offload_models(text_encoding_pipeline, device=accelerator.device, offload=args.offload):
            instance_bundle = compute_text_embeddings(
                args.instance_prompt, static_grid_h, static_grid_w, text_encoding_pipeline
            )

    # Handle class prompt for prior-preservation.
    if args.with_prior_preservation:
        with offload_models(text_encoding_pipeline, device=accelerator.device, offload=args.offload):
            class_bundle = compute_text_embeddings(
                args.class_prompt, static_grid_h, static_grid_w, text_encoding_pipeline
            )
    ...
    if not train_dataset.custom_instance_prompts:
        static_bundle = instance_bundle
        if args.with_prior_preservation:
            # concat instance+class along the batch dim for each of (embeds, position_ids, segment_ids, indicator)
            static_bundle = tuple(torch.cat([inst, cls], dim=0) for inst, cls in zip(instance_bundle, class_bundle))
        ...
        for batch in tqdm(train_dataloader, desc="Caching latents"):
            with torch.no_grad():
                ...
                if train_dataset.custom_instance_prompts:
                    # grid for this batch's actual image size (drives the packed layout); each batch is a
                    # single bucket, so this is correct under aspect-ratio bucketing.
                    grid_h = batch["pixel_values"].shape[-2] // patch_pixels
                    grid_w = batch["pixel_values"].shape[-1] // patch_pixels
                    if args.fsdp_text_encoder:
                        bundle = compute_text_embeddings(batch["prompts"], grid_h, grid_w, text_encoding_pipeline)
                    else:
                        with offload_models(text_encoding_pipeline, device=accelerator.device, offload=args.offload):
                            bundle = compute_text_embeddings(batch["prompts"], grid_h, grid_w, text_encoding_pipeline)
                    bundle_cache.append(bundle)
    ...
    if accelerator.is_main_process:
        tracker_name = "dreambooth-ideogram4-lora"
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                if train_dataset.custom_instance_prompts:
                    prompt_embeds, position_ids, segment_ids, indicator = bundle_cache[step]
                else:
                    # collate_fn orders prior-preservation batches as [inst1..instB, class1..classB];
                    # repeat each static entry along dim 0 to match. Read from the stable `static_bundle`.
                    num_repeat_elements = len(prompts) // 2 if args.with_prior_preservation else len(prompts)
                    prompt_embeds, position_ids, segment_ids, indicator = (
                        t.repeat_interleave(num_repeat_elements, dim=0) for t in static_bundle
                    )
                ...
                # Pack the VAE latent into the transformer's token layout, then bn-normalize.
                # The packed conditioning (prompt_embeds + position/segment/indicator) was built for this
                # same grid by encode_prompt, so the sequence lengths line up.
                image_height = model_input.shape[-2] * vae_scale_factor
                image_width = model_input.shape[-1] * vae_scale_factor
                model_input = patchify_latents(model_input, patch_size)  # (B, grid_h*grid_w, latent_dim)
                model_input = (model_input - latents_bn_mean) / latents_bn_std

                bsz, num_image_tokens, latent_dim = model_input.shape

                # Sample noise that we'll add to the latents
                noise = torch.randn_like(model_input)

                # Sample sigma per image. Ideogram4 uses a resolution-aware logit-normal schedule (defaults
                # below), so we pass the pixel-count-shifted mean to compute_density_for_timestep_sampling and
                # use its output directly as sigma -- unlike most scripts we do NOT index the FlowMatch
                # scheduler grid, which carries a shift Ideogram4's schedule does not use. sigma in (0, 1).
                sigmas = compute_density_for_timestep_sampling(
                    weighting_scheme=args.weighting_scheme,
                    batch_size=bsz,
                    logit_mean=_resolution_aware_mu(image_height, image_width, base_mu=args.logit_mean),
                    logit_std=args.logit_std,
                    mode_scale=args.mode_scale,
                    device=accelerator.device,
                )
                sigmas = sigmas.to(dtype=model_input.dtype).view(bsz, 1, 1)

                # Add noise according to flow matching: zt = (1 - sigma) * x0 + sigma * noise.
                noisy_model_input = (1.0 - sigmas) * model_input + sigmas * noise

                # Pack the noisy image tokens behind zeroed text positions to form the full sequence
                # the transformer expects: [text padding (max_sequence_length) ++ image tokens].
                text_latent_padding = torch.zeros(
                    bsz,
                ...
                packed_hidden_states = torch.cat([text_latent_padding, noisy_model_input], dim=1)
                ...
                model_pred = model_pred[:, args.max_sequence_length :]

                # these weighting schemes use a uniform timestep sampling
                # and instead post-weight the loss
                weighting = compute_loss_weighting_for_sd3(weighting_scheme=args.weighting_scheme, sigmas=sigmas)

                # ideogram predicts v=x0−noise
    ...
    if accelerator.is_main_process:
        ...
        if is_fsdp:
            ...
            transformer = unwrap_model(transformer)
            # Skip the dtype cast for a quantized base (passed config or a pre-quantized nf4 checkpoint):
            # casting a bitsandbytes model is unsupported, and we only extract the (unquantized) LoRA layers anyway.
            if args.bnb_quantization_config_path is None and not getattr(transformer, "is_quantized", False):
                if args.upcast_before_saving:
